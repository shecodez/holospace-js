...
class JoinServer extends React.Component {
  ...
  submit = data => {
    ...
    return this.props.createMembership(invitation)
			.then((membership) => {
				this.toggleModal();
		...
		return (
			<Modal size={'tiny'} open={isOpen} onClose={this.toggleModal}>
				<Modal.Content style={{ textAlign: 'center' }}>
					...
					<MembershipForm submit={this.submit} back={this.toggleModal} />
				</Modal.Content>
			</Modal>
		);
	}
...
export default connect(null, { createMembership })(JoinServer);
